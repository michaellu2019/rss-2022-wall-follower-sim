...
class WallFollower:
    ...
    def callback(self, data):
        ...
        if self.SIDE == self.SIDE_LEFT:
            self.distances[0] = self.best_fit_line(self.left_scan_ranges, self.left_scan_angles, side=self.SIDE_LEFT, color=(1.0, 1.0, 1.0), visualize=self.visualize_side == self.SIDE_LEFT)
        else:
            self.distances[2] = self.best_fit_line(self.right_scan_ranges, self.right_scan_angles, side=self.SIDE_RIGHT, color=(0.0, 0.0, 1.0), visualize=self.visualize_side == self.SIDE_RIGHT)
